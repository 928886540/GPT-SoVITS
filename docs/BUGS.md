...
Status: patched in v2028881932; live/history policy superseded by BUG-036 v2028881933, needs real Tavo regression
...
Guard: v1932 的 AudioContext keepalive 仍保留：出现 `音频通道未放行` 后，下一次用户手势必须创建新 AudioContext，不能让所有卡片永久失败。流式生成从点击到首段 PCM 的等待期间，控制台应出现 `AudioContext keepalive started`，到 `Web Audio 播放时钟已启动` 后出现 keepalive stopped。后台/切卡最终策略以 BUG-036 为准：saved/history 不主动暂停；live/pending 不再恢复，切走或中断时删除未完成任务。

## BUG-036: live 流式和历史音频后台/切卡策略混用

Status: patched in v2028881933, needs real Tavo regression
...
Fix: v1933 改成分离策略：saved/history 音频不再被前端主动 `visibilitychange/pagehide` 暂停；live/pending 流式在上一条/下一条前弹确认，确认后停止 Web Audio/element audio、删除服务端任务/cache、移除当前 live 卡片，再切目标历史；live/pending 在 `visibilitychange/pagehide`、用户暂停、`audio_suspended`、首段超时、连续缓冲或网络流中断时直接中止并删除，不尝试恢复；消息历史持久化和懒加载计数只保存/显示已成功落盘的 saved 音频。

Guard: 历史音频播放时切 Tavo 页面或系统后台，前端不应主动 pause/reset；如果宿主允许后台播放，应继续播放，若宿主自行暂停，用户回来点播放再继续。live 流式播放时切上一条/下一条必须明确提示“会中断并删除本次流式任务”；确认后当前 live 卡片消失且不会保存成历史。live 未成功落盘前重进消息，不能恢复为历史音频；只有 `/tts_dialogue_job_status` done 或 saved track 才能写入 `sovits_tracks_<messageId>`。
