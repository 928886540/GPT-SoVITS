...
Fix: 当前正则入口改为 `https://sovits.928886540.xyz/static/tavo.js?v=2028881919`；loader 版本升到 `20260602-sovits-bridge-v29`；runtime parts 版本升到 `20260602-sovits-bridge-v11`。保存/应用 Tavo 正则后，必须回到真实聊天消息重新渲染，不能只改仓库 JSON。

Guard: 真实 Tavo 控制台脚本来源必须显示 `https://sovits.928886540.xyz/static/tavo.js?v=2028881919`。点击生成前确认同源 `/health`、`/static/tavo.runtime.manifest.json`、`/static/tavo.runtime.js` 和 `/parse_text` 可达；如果控制台仍显示旧子域名或旧 `v=2028821788`，先清正则/消息渲染缓存，不继续查 LLM。
...
Fix: 正则入口改为 `https://sovits.928886540.xyz/static/tavo.js?v=2028881919`；loader/runtime 版本同步 bump。运行时新写入 `sovits_tracks_*` 和 `sovits_tavo_audio_v1`，并保留旧 key/旧 IndexedDB 的读取兼容，避免历史播放记录和本机离线音频直接丢失。测试页和 active runtime 文案/console/MediaSession/data attr 改成小写 `sovits`。
...
Root cause: 之前把 `curl`/普通浏览器可达误判成 Tavo WebView 可达。真实问题是 Tavo 的 `about:srcdoc`/AR 沙箱对从 opaque origin 发起的跨源 JSON POST 不稳定或直接拦截；`<script src>` 和部分 GET 能加载，不代表 `fetch(..., {method:"POST", headers:{"Content-Type":"application/json"}})` 能跑通。

Fix: 新增同源隐藏 iframe bridge：父页面仍由 Tavo `about:srcdoc` 加载脚本，但 JSON POST/DELETE 交给 `https://sovits.928886540.xyz/static/tavo.fetch.bridge.html` 内部执行；bridge 页面与 adapter 同源，请求 `/parse_text`、`/tts_*_job` 时不再跨 origin。父页面通过 `postMessage` 收发 Response-like 结果。正则入口升到 `v=2028881919`，loader `20260602-sovits-bridge-v29`，runtime parts `20260602-sovits-bridge-v11`。

Guard: 真实 Tavo 控制台应出现 bridge 日志；`POST /parse_text` 必须到达 adapter 日志。`/parse_text` 若失败，也应该变成后端 HTTP 错误或 LLM proxy 错误，不应再是浏览器 `TypeError: Load failed`。
