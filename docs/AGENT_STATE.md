...
## BUG-022 真实 Tavo 仍加载旧 HTTPS 脚本（2026-06-02 21:15 +08:00）

用户贴出的真实 Tavo 日志显示：

- 脚本来源：`https://index-tts.928886540.xyz/static/tavo.js?v=2028821788`
- `/parse_text` 请求 URL：`https://index-tts.928886540.xyz/parse_text`
- 错误：`TypeError: Load failed`

当前本机事实：

- 仓库正则已是 `http://192.168.8.100:9880/static/tavo.js?v=2028881917`。
- `http://127.0.0.1:9880/health` 正常。
- `http://192.168.8.100:9880/health` 正常。
- 本机 curl 访问 `https://index-tts.928886540.xyz/static/tavo.js?v=2028821788` 和 `/parse_text` 均连接重置。

结论：这不是 LLM endpoint `127.0.0.1:8317` 的问题，也不是后端 parse 逻辑问题；真实 Tavo 仍在使用旧 HTTPS/旧版本脚本，浏览器请求没有打到当前 LAN adapter。

下一步：在真实 Tavo 正则里刷新脚本 URL 到 `http://192.168.8.100:9880/static/tavo.js?v=2028881917`，保存/应用后重新渲染真实聊天消息。刷新后控制台脚本来源必须变成 `192.168.8.100:9880`，再继续验证 BUG-019/020/021。
