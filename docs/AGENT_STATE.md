...
## sovits 小写命名与 HTTPS 映射（2026-06-02 21:24 +08:00）
...
- 正则入口改为 `https://sovits.928886540.xyz/static/tavo.js?v=2028881918`。
- Loader 版本改为 `20260602-sovits-v28`，runtime parts/manifest 改为 `20260602-sovits-v10`。
- 新历史 key 使用 `sovits_tracks_*`，新 IndexedDB 使用 `sovits_tavo_audio_v1`；旧 key/旧 DB 只做读取兼容，避免历史音频丢失。

下一步：

1. 已通过本地验证：`node --check static\tavo.js`、`node --check static\tavo.runtime.js`、manifest concat `new Function`、`python -m py_compile gsv_tavo_adapter.py`、`git diff --check`、key 扫描、活跃代码/文档旧品牌扫描。
2. 已通过 HTTPS 映射验证：`/health` 200；`/static/tavo.js?v=2028881918` 返回 loader `20260602-sovits-v28`；part 05 返回 `sovits_tracks_*` / `sovits_tavo_audio_v1`；`POST /parse_text {}` 返回 422，说明请求到达 adapter。
3. 待真实 Tavo 回归：正则必须刷新到 `https://sovits.928886540.xyz/static/tavo.js?v=2028881918` 并重新渲染消息；控制台脚本来源不变就先处理 Tavo 缓存，不查 LLM。
...
- 仓库正则已改为 `https://sovits.928886540.xyz/static/tavo.js?v=2028881918`。

结论：这不是 LLM endpoint `127.0.0.1:8317` 的问题，也不是后端 parse 逻辑问题；真实 Tavo 仍在使用旧 HTTPS/旧版本脚本，或 WebView 正则/消息渲染缓存未刷新到当前 `sovits` 子域名。

下一步：在真实 Tavo 正则里刷新脚本 URL 到 `https://sovits.928886540.xyz/static/tavo.js?v=2028881918`，保存/应用后重新渲染真实聊天消息。刷新后控制台脚本来源必须变成 `sovits.928886540.xyz`，再继续验证 BUG-019/020/021。
