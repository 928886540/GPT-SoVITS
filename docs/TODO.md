...
- 当前最新待真实 Tavo 回归版本：正则 `https://sovits.928886540.xyz/static/tavo.js?v=2028881927`，loader `20260603-history-restore-v37`，runtime parts/manifest `20260603-history-restore-v19`。优先回归 BUG-030/031/032：重进消息只恢复历史 metadata、不自动读取音频；历史条数显示一致；live 流式点歌词不能重连；播放失败不能再显示裸 `Error`。
