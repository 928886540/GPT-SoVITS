...
- 当前最新待真实 Tavo 回归版本：正则 `https://sovits.928886540.xyz/static/tavo.js?v=2028881932`，loader `20260603-audio-keepalive-v42`，runtime parts/manifest `20260603-audio-keepalive-v24`。优先回归 BUG-035：流式生成等待首段期间必须有 AudioContext keepalive，不能报 `音频通道未放行`；切 Tavo 其他页面/桌面时当前音频显式暂停并保存断点，返回后点播放只续播同一条，不叠第二路。继续覆盖 BUG-034：删除所有历史后计数必须立即变 `历史音频 0 条` 且重进不回灌旧 4 条。
