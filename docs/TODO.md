...
- 当前最新待真实 Tavo 回归版本：正则 `https://sovits.928886540.xyz/static/tavo.js?v=2028881929`，loader `20260603-state-model-v39`，runtime parts/manifest `20260603-state-model-v21`。优先回归 BUG-033：历史条数显示为 `历史音频 N 条`；上一条/下一条只切卡并立即刷新页码，不自动读取/播放；设置页覆盖在播放器卡片位置；saved 音频播放失败必须显示具体阶段；live 歌词在没有真实时间轴前不得假同步或跳转。
