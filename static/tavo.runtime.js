;(function () {
  ...
  var RUNTIME_PARTS_VERSION = "20260602-sovits-server-error-v18";
  ...
  function fallbackManifest(reason) {
    try { console.warn("[GPT-SoVITS TAVO runtime loader] manifest fallback", reason || ""); } catch (_) {}
    return {
      schema: 1,
  ...
  function fetchManifest() {
    ...
    return fetch(url, { cache: "no-store" }).then(function (res) {
      if (!res.ok) throw new Error("runtime manifest load failed: " + res.status + " " + url);
      return res.json();
  ...
  window.__gptsovits_tavo_runtime_ready = fetchManifest().then(function (manifest) {
    try {
      window.__gptsovits_tavo_runtime_manifest = manifest;
      console.log("[GPT-SoVITS TAVO runtime loader] manifest", manifest.mode, manifest.runtimeVersion, manifest.modules.length + " modules");
    } catch (_) {}
