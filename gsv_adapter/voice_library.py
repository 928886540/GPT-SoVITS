...
def list_voices() -> list[dict]:
    """List GPT-SoVITS voice profiles and fallback audio references.

    A full profile is a JSON file under ``prompts/library``. Audio-only files
    are exposed as lightweight profiles so the old Tavo picker can work before
    dedicated GPT-SoVITS profile JSON exists.
    """
    ...
    for path in library_dir.rglob("*"):
        ...
        if ext not in VOICE_LIB_EXTS and ext != VOICE_PROFILE_EXT:
            continue
        try:
        ...
        item = {
            ...
            "subdir": subdir,
            "kind": "profile" if ext == VOICE_PROFILE_EXT else "audio",
        }
        if ext == VOICE_PROFILE_EXT:
            ...
            item["hidden_from_picker"] = bool(is_aux_profile or profile.get("hidden_from_picker"))
            item["usable_for_gptsovits"] = bool(
                profile.get("ref_audio_path")
                and profile.get("prompt_text")
                and not is_aux_profile
                and profile.get("usable_for_gptsovits", True) is not False
            )
        else:
...
def get_voice_profile(name: str) -> Optional[dict]:
    """Return a GPT-SoVITS voice profile by name or audio fallback."""
    safe = safe_voice_name(name)
    if not safe:
        return None

    library_dir = Path(VOICE_LIB_DIR)
    profile_path = library_dir / f"{safe}{VOICE_PROFILE_EXT}"
    if profile_path.is_file():
        profile = _read_profile(profile_path)
        profile.setdefault("name", safe)
        profile.setdefault("profile_path", _format_voice_path(profile_path))
        return profile

    ref_audio_path = get_voice_path(safe)
